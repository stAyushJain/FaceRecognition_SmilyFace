...
class GooglyFaceTracker extends Tracker<Face> {
    private static final float EYE_CLOSED_THRESHOLD = 0.25f;
    ...
    public void onUpdate(FaceDetector.Detections<Face> detectionResults, Face face) {
        ...
        mEyesGraphic.updateEyes(face,leftPosition, isLeftOpen, rightPosition, isRightOpen);

        PointF bottomMouthPos = getLandmarkPosition(face, Landmark.BOTTOM_MOUTH);
        PointF leftCheekPos = getLandmarkPosition(face, Landmark.LEFT_CHEEK);
        PointF leftEarPos = getLandmarkPosition(face, Landmark.LEFT_EAR);
        PointF leftEarTipPos = getLandmarkPosition(face, Landmark.LEFT_EAR_TIP);
        PointF leftMouthPos = getLandmarkPosition(face, Landmark.LEFT_MOUTH);
        PointF nodeBasePos = getLandmarkPosition(face, Landmark.NOSE_BASE);
        PointF rightCheekPos = getLandmarkPosition(face, Landmark.RIGHT_CHEEK);
        PointF rightEarPos = getLandmarkPosition(face, Landmark.RIGHT_EAR);
        PointF rightEarTipPos = getLandmarkPosition(face, Landmark.RIGHT_EAR_TIP);
        PointF rightMouthPos = getLandmarkPosition(face, Landmark.RIGHT_MOUTH);
        mEyesGraphic.updateFaceFeatures(bottomMouthPos, leftCheekPos, leftEarPos, leftEarTipPos,
                leftMouthPos, nodeBasePos, rightCheekPos, rightEarPos, rightEarTipPos, rightMouthPos);
    }
